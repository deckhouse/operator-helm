...
## Развёртывание версии, опубликованной в OCI-репозитории

Классический HTTP-репозиторий может публиковать часть версий своих чартов в OCI-репозитории. Такая запись указывает артефакт в `urls` вместо ссылки на архив `.tgz`:
...
Когда аддон запрашивает такую версию, контроллер скачивает её из OCI-репозитория, а не из индекса классического репозитория. OCI-репозиторий должен быть доступен для чтения без аутентификации: учётные данные репозитория не отправляются на хост, который назван только в его индексе.
