...
- Развёртывание версии чарта, которую классический репозиторий публикует в OCI-регистри: если индекс репозитория указывает версию в регистри, эта версия скачивается из регистри, хотя её репозиторий — классический.
...
- Креденшлы и TLS-настройки репозитория не отправляются в регистри, который назван только в его индексе: версия чарта, опубликованная в регистри на другом хосте, скачивается анонимно, поэтому такой регистри должен быть доступен для чтения без аутентификации.
