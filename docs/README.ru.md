...
- Учётные данные и TLS-настройки репозитория не отправляются в OCI-репозиторий, который назван только в его индексе: версия чарта, опубликованная в OCI-репозитории на другом хосте, скачивается анонимно, поэтому такой OCI-репозиторий должен быть доступен для чтения без аутентификации.
