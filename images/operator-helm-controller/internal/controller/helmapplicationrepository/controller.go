...
// Package helmapplicationrepository wires the shared repository reconciler to the
// HelmApplicationRepository kind. The kind is namespaced while its internal objects
// live in the operator namespace, so the watches on those objects map back through
// both source labels.
...
const (
	ControllerName = "helmapplicationrepository-controller"
)
...
func SetupWithManager(mgr ctrl.Manager) error {
	...
	r := reconcile.New(client, reconcile.Deps{
		...
		Catalog:       services.NewRepoSyncService(client, mgr.GetScheme(), repoclient.NewClient, adapter.NewApplicationCatalog(client)),
	...
	mapInternal := utils.MapNamespacedInternalResources(
		...
		helmv1alpha1.HelmApplicationRepositoryLabelSourceName,
